...
A Solution
If I can find out the height of the node and whether the node is left child or right child, I can easily find the parent of the node.
For left children, parent = node_label + 2**node_height.
For right children, parent = node_label + 1.
If height on node matches height of tree then return -1.
Calculating Height

For any given tree of height = h, the root will be 2^h -1.
Thus if node + 1 is perfect power of 2 then we can find out the height easily. (No need to take log base 2, just see the number of bits after converting to binary).
If its not perfect power of 2, then calculate x = 2^i - 1. Choose i = floor(log_base_2(node)).
Subtract x from node.
Repeat step 3 and 4 until node + 1 becomes perfect power of 2.
Once node + 1 is perfect power of 2, calculate height = log_base_2(node + 1).
Left or Right Child

If node + 1 was already a perfect power of 2, then it was left child.
Just before becoming a perfect power of 2, if node was equal to 2 * x, then it was right child.
Otherwise it was left child.
Summary
