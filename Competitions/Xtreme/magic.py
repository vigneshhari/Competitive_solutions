...
somelanguage = { "xrtp": 0, "pmr": 1, "yep": 2, "yjtrr": 3, "gpit": 4, "gobr": 5, "doc": 6, "drbrm": 7, "rohjy": 8, "momr": 9, "yrm": 10, "rarbrm": 11,
"yerabr": 12, "yjotyrrm": 13, "gpityrrm": 14, "gogyrrm": 15, "docyrrm": 16, "drbrmyrrm": 17, "rohjyyrrm": 18, "momryrrm": 19, "yermyu": 20 }

def TheWorstProblemEver(N):
    shit = 1
    for n in range( int(N) ):
        arr = [ str(somelanguage[x]) for x in input().split() ]
        shit *= int("".join(arr), 16)
    print (shit)

TheWorstProblemEver(input())
